...
export function Cart(props) {
    ...
    const handleCouponChange = React.useCallback((e) => {
        const discount = parseInt(e.target.value);
        const discountPercentage = 100 - discount;

        props.setCartState({
            ...props.cart,
            discount: ((discount / 100) * props.cart.subTotal),
            selectedCoupon: discount,
            totalPrice: (props.cart.subTotal * discountPercentage) / 100
        });
    }, [props.cart]);
        ...
        <div className="card outlined my-16 mr-25 flex-30">
                        ...
                        props.cart.items.map((cartItem, idx) => {
                            return (
                                <tr data-testid={'cart-item-' + idx}
                                    ...
                                    <td>{idx + 1}.</td>
                                    <td className="name" data-testid="cart-item-name">{cartItem.heading}</td>
            ...
            <div className="layout-row justify-content-between align-items-center px-8 mx-12">
                ...
                <select data-testid="cart-coupon"
                    className="coupon-select" onChange={handleCouponChange}>
